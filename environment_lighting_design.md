...
> 状态：设计文档（2026-06-16）。Stage 1（仅太阳光、黑背景数据集）已达优秀水平
> （uniform 数据集 ~33.3 PSNR，held-out 太阳零泛化差距）。本文档定义 Stage 2：
> 冻结已标定的高斯点集，以预计算辐射传输（PRT）的形式叠加环境光。
...
**场景现状（2026-06-16 经 MCP 核实 /Game/Maps/Cloud）**：场景已配
`SkyAtmosphere`(visible) + `SkyLight`(bAffectsWorld, RealTimeCapture,
LowerHemisphereIsBlack) + `SceneCaptureCube → /Game/VolumetricCloud/RT_SkyCube`
（HDR, bCaptureEveryFrame）。**环境 cubemap 捕获机制已现成**，采集侧不需新建。
云本体是 `HeterogeneousVolume`（非 VolumetricCloud 组件）。
...
将拥有 `img_env_on`（云可见、含环境光），已有 `img_env_off`（Stage 1 数据）。

```
环境贡献_GT = img_env_on − img_env_off
```

这是**纯环境光贡献**，把已标定的太阳项完全解耦。**冻结太阳模型 + V_lm，只用
差分图监督环境项的少量自由度** ⇒ 任何残差纯属环境模型本身，不污染太阳部分。
与 tonemap/octave 实验同一方法论：先隔离、再合并。

> 备选监督：直接用 `img_env_on` 全图（太阳项冻结，loss 只能动环境自由度）。
> 差分图更干净（去掉太阳项的大基底，信噪比高），优先差分。
...
- 网关：`if getattr(pipe,"env_lighting",False) and pc 有 transfer`：
  `Lk = sun_term + pc.apply_env(viewpoint_camera.sky_sh)`。
- 逐帧 `sky_sh` 从 camera 取（dataset_readers 注入，同 `sun_dir` 路径）。
...
- 扩 `cloud_dataset_generator.py`（手动 py 模式，避开会崩的 MCP 插件）：
  每太阳方向 →（a）**隐藏 HeterogeneousVolume** → 捕获 `RT_SkyCube` → 导出/投 SH；
  （b）恢复云体 → 渲 env-on 训练图（云可见、含环境光）。env-off 已有。
...
4. **监督信号**：差分图（env_on − env_off）还是直接 env_on 全图？（倾向差分）
