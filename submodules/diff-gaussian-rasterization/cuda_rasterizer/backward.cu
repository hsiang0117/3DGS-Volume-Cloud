	...
	bool antialiasing)
{
	...
	if(antialiasing)
	{
		...
		const float dL_dpacked_input_v = dL_dpacked_input[idx];
		const float d_h_convolution_scaling = dL_dpacked_input_v * packed_inputs[idx];
		dL_dpacked_input[idx] = dL_dpacked_input_v * h_convolution_scaling;
...
)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float packed_pixel = con_o.w * G;
			const float alpha_raw = use_analytic_tau ? (1.0f - exp(-packed_pixel)) : packed_pixel;
			...
			const float dL_dpacked_pixel = use_analytic_tau ? (exp(-packed_pixel) * dL_dalpha) : dL_dalpha;
			const float dL_dG = con_o.w * dL_dpacked_pixel;
			...
			// Update gradients w.r.t. the packed Gaussian scalar.
			if (use_analytic_tau)
				atomicAdd(&(dL_dtau[global_id]), G * dL_dpacked_pixel);
			else
				atomicAdd(&(dL_dopacity[global_id]), G * dL_dpacked_pixel);
		}
	}
}
...
// Backward of the light-space front-tau recording (record_front_tau).
//
// Forward (per pixel, front to back, analytic-tau branch):
//     w_j   = alpha_j * T_j                     (blend weight)
...
lightpassBackwardCUDA(
	...
	const float* __restrict__ final_Ts,
	const uint32_t* __restrict__ n_contrib,
	const float* __restrict__ grad_tau_front_sum,
	float* __restrict__ dL_dtau)
{
	auto block = cg::this_thread_block();
	const uint32_t horizontal_blocks = (W + BLOCK_X - 1) / BLOCK_X;
	...
	__shared__ float2 collected_xy[BLOCK_SIZE];
	__shared__ float4 collected_conic_opacity[BLOCK_SIZE];
	__shared__ float collected_gsum[BLOCK_SIZE];
	...
	// Running sum of w_j * gsum_j over Gaussians BEHIND the current one.
	float S = 0.0f;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_xy[block.thread_rank()] = points_xy_image[coll_id];
			collected_conic_opacity[block.thread_rank()] = conic_opacity[coll_id];
			collected_gsum[block.thread_rank()] = grad_tau_front_sum[coll_id];
		}
		block.sync();

		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			// Skip Gaussians the forward never blended at this pixel
			// (behind early termination).
			contributor--;
			if (contributor >= last_contributor)
				continue;
			...
			const float alpha = min(0.99f, 1.0f - exp(-tau_pixel));
			if (alpha < 1.0f / 255.0f)
				continue;

			// Recover the forward's pre-blend transmittance for this
			// Gaussian (stock backward recurrence).
			T = T / (1.f - alpha);

			// This Gaussian's tau attenuates every recording BEHIND it.
			if (S != 0.0f)
				atomicAdd(&dL_dtau[collected_id[j]], G * S);

			// Fold its own recording weight in for Gaussians in front.
			S += alpha * T * collected_gsum[j];
		}
	}
}

void BACKWARD::lightpass(
	...
	float* dL_dtau)
{
	lightpassBackwardCUDA << <grid, block >> > (
		ranges,
		point_list,
		W, H,
		means2D,
